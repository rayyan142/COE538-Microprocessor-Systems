;******************************************************************
;* This program is for the mc9s12c32 microcontroller,             *
;******************************************************************
...
; subroutine section
;*******************************************************************
DISPATCHER  CMPA  #START                    ; If it is the START state -----------------
            BNE   NOT_START                 ;                                           |
...
NOT_FWD     CMPA  #REV                      ;                                           |
            ...
            JMP   DISP_EXIT                 ;                                           S
                                            ;                                           P
NOT_REV     CMPA  #ALL_STP                  ;                                           A
            BNE   NOT_ALL_STOP              ;                                           T
            JSR   ALL_STP_ST                ;                                           C
            JMP   DISP_EXIT                 ;                                           H
                                            ;                                           E
NOT_ALL_STOP CMPA #FWD_TRN                  ;                                           R
            BNE   NOT_FWD_TRN               ;                                           |
            JSR   FWD_TRN_ST                ;                                           |
            JMP   DISP_EXIT                 ;                                           |
                                            ;                                           |
NOT_FWD_TRN CMPA  #REV_TRN                  ; Else if it is the REV_TRN state           |
            BNE   NOT_REV_TRN               ;                                           |
...
;*******************************************************************
initLCD     BSET  DDRB,%11111111            ; configure pins PB7,PB6,...,PB0 for output
...
int2BCD     XGDX                            ; Save the binary number into .X, X <=> D
            ...
            XGDX                            ; Not zero, get the binary number back to .D as dividend, X <=> D
            LDX   #10                       ; Setup 10 (Decimal!) as the divisor
            IDIV                            ; Divide: Quotient is now in .X, remainder in .D
            STAB  UNITS                     ; Store remainder, lower half of AccD is AccB
...
C_TTHOU     LDAA  TEN_THOUS                 ; Check the ten_thousands digit
            ORAA  NO_BLANK                  ; ORAA with 0 to potentially set Z-flag
            BNE   NOT_BLANK1                ; If resultant byte is not 0, branch to NOT_BLANK1
...
UPDT_DISPL  MOVB  #$90,ATDCTL5              ; r.just., unsign., sing.conv., mult., ch0, start conv.
            ...
            LDAB  #39                       ; AccB = 39
            MUL                             ; AccD = AccA x AccB = 1st result x 39
            ADDD  #600                      ; AccD = 1st result x 39 + 600 = 1000V_batt
